...
import { SaveStudentCalendarPopupComponent } from '../save-student-calendar-popup/save-student-calendar-popup.component';
import { RemoveStudentCalendarPopupComponent } from '../remove-student-calendar-popup/remove-student-calendar-popup.component';
...
@Component({
  selector: 'app-instructor-calendar-day',
  templateUrl: './instructor-calendar-day.component.html',
  styleUrls: ['./instructor-calendar-day.component.css']
})
export class InstructorCalendarDayComponent implements OnInit {
  ...
  openDialog(){
    ...
    this.dialogRef.open(SaveStudentCalendarPopupComponent, {autoFocus: false,
      data: {
  ...
  removeStudentDialog(selectedLesson: StudentsHourDrive){
    this.dialogRef.open(RemoveStudentCalendarPopupComponent, {
      data: {
  ...
  hoursListfunction(){
    ...
    for(var i=Math.trunc(this.daysAndHours[getDayValue].firstHour-1); i<Math.trunc(this.daysAndHours[getDayValue].lastHour); i++){
      ...
      for(var j=0; j<this.studentsWithTodaysDate.length; j++){
        ...
        if(Math.trunc(this.studentsWithTodaysDate[j].mainHourTo) == i+1 && Math.trunc(this.studentsWithTodaysDate[j].mainHourFrom) != i+1){
