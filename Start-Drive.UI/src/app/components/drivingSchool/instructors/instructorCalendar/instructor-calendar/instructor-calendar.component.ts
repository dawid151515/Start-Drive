...
import { FreeDaysInstructorPopupComponent } from '../free-days-instructor-popup/free-days-instructor-popup.component';
...
export class InstructorCalendarComponent implements OnInit  {
  ...
  ngOnInit(): void {
    ...
    this.setInstructorDaysOffAbsenceModel()
  ...
  setCurrentRefreshData(){
    //refresh absence
    var refreshMonth = localStorage.getItem('month')
    var refreshYear = localStorage.getItem('year')
  ...
  setCurrentRefreshDay(){
    var isDay = localStorage.getItem('day')
    if(isDay != null){
      ...
      if(localStorage.getItem('scrollPositionDay') != null){
        setTimeout(() => {
          var scroll = Number(localStorage.getItem('scrollPositionDay'))
          window.scroll(0, scroll)
          localStorage.removeItem('scrollPositionDay')
  ...
  //table with instructor absences
  @Input()
  ...
  calendarStudentsDriveDate(){
    ...
    //selecting tasks for a specific month
    ...
    //listing the hours occupied for each day of the month
    var busyHours = []
    ...
    //listing the free hours for each day of the month on the table
    var counterOnDay = 0
    var dayOfTheMonth = 0
  ...
  setStyleAbsence(day: number): string{
    var date = this.convertDateString(this.currentYearValue, this.currentMonthValue, `${day}`)
    if(this.freeDaysInstructor[date] != undefined){
      var typeOfAbsence = this.freeDaysInstructor[date]

      if(typeOfAbsence.split(' ')[0].toLocaleLowerCase() == 'urlop'){
        return 'background-color: #00FFFF'
      }else if(typeOfAbsence.split(' ')[0].toLocaleLowerCase() == 'chorobowe'){
  ...
  instructorDaysOffAbsenceModel: InstructorAbsence[] = []
  setInstructorDaysOffAbsenceModel(){
    if(this.selectedInstructorParent.instructorDaysOff != undefined){
      this.instructorDaysOffAbsenceModel = this.selectedInstructorParent.instructorDaysOff
    }
  }
  ...
  saveFreeDay: boolean = false
  openDialog(){
    this.dateToSend = this.convertDateString(`${this.currentYearValue}`, `${this.currentMonthValue}`, `1`)

    this.dialogRef.open(FreeDaysInstructorPopupComponent, {
      data: {
        month: this.currentMonthValueNumber,
        year: this.currentYearValueNumber,
        instructorAbsence: this.freeDaysInstructor,
  ...
  @Input()
  schoolId: number = -30
  instructorId: number = -29
  setInstructorId(){
    if(this.selectedInstructorParent.id != undefined){
      this.instructorId = this.selectedInstructorParent.id
    }
  }
}
